...
saveButton.addEventListener('click', () => {
  ...
  const loop = loopControl.checked;

  const wavBlob = bufferToWav(audioBuffer, speed, volume, loop);
...
function bufferToWav(buffer, speed = 1, volume = 1, loop = false) {
  const numOfChan = buffer.numberOfChannels;
  const sampleRate = buffer.sampleRate + speed;
  const length = Math.floor(buffer.length / speed);
  const totalLength = loop ? length * 3 : 0.5; //length;
  const wavLength = totalLength * numOfChan * 2 + 44;
  ...
  writeString('RIFF');
  view.setUint32(offset, 36 + totalLength * numOfChan * 2, true);
  offset += 4;
  writeString('WAVE');
  writeString('fmt ');
  ...
  offset += 2;
  writeString('data');
  view.setUint32(offset, totalLength * numOfChan * 2, true);
  offset += 4;

  for (let loopCount = 0; loopCount < (loop ? 3 : 1); loopCount++) {
